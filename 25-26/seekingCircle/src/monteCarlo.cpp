...
namespace MonteCarlo
{
    ...
    void init(pose start, int max_samples)
    {
        ...
        samples.reserve(J);
        samples.resize(J);
        std::normal_distribution<float> rdn{0, 2.5};
    ...
    void update()
    {
        ...
        static float prevSpread = spread;
        ...
        auto rdn2 = std::normal_distribution<float>(0, 0.5);
        ...
        // else
        // {
        auto rdn4 = std::normal_distribution<float>(0, spread / 4);
        center.theta = angle;
        center.x += delta.x;
        center.y += delta.y;
        for (int i = 0; i < J; i++)
        {
            // std::cout<<i<<std::endl;
            float rx = center.x + rdn4(randgen);
            float ry = center.y + rdn4(randgen);
            float rt = center.theta;
            samples[i] = sample(rx, ry, rt, 1.0f / J);
            // samples[i].print();
        }
        for (int i = 0; i < J; i++)
        ...
        spread = 0.1 / (meanWeight() * 5);
        spread = fmax(spread, 0.01);
        spread = fmin(spread, 12);
        spread = prevSpread + 0.15 * (spread - prevSpread);
        float sum = 0;
        float sumw = 0;
        for (int i = 0; i < J; i++)
        {
            sum += samples[i].x * samples[i].w;
            sumw += samples[i].w;
        }
        auto rdn5 = std::normal_distribution<float>(sum / sumw, spread / 12);
        center.x = sum / sumw;
        sum = 0;
        sumw = 0;
        for (int i = 0; i < J; i++)
        {
            sum += samples[i].y * samples[i].w;
            sumw += samples[i].w;
        }
        auto rdn6 = std::normal_distribution<float>(sum / sumw, spread / 12);
        center.y = sum / sumw;
    ...
    void renderParticles(SDL_Renderer *renderer)
    {
        ...
        for (int i = 0; i < J; i++)
        {
            ...
            if (i == 0)
            {
                drawCircle(renderer, x, y, spread * SCALE_FACTOR);
            }
            else
            {
                drawCircle(renderer, x, y, 1 * SCALE_FACTOR);
            }
        }
    }
};